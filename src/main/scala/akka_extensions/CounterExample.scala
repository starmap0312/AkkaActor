...
class Counter extends Extension { // the Counter implementaion of Extension
  ...
  logger.info("Counter Extension is instantiated")
  println("Counter Extension is instantiated")
...
object Counter extends ExtensionId[Counter] with ExtensionIdProvider {
  val logger = LoggerFactory.getLogger(getClass.getName)
  logger.info("Counter ExtensionId is instantiated")
  println("Counter ExtensionId is instantiated")
...
object CounterExample extends App {
  ...
  val counter1: Counter = Counter.get(system) // a Counter singleton is demanded, thus instantiated and shared within the system
  val counter2: Counter = Counter.get(system) // as it's shared within the ActorSystem, no new Counter is instantiated here
  val counter3: Counter = Counter(system) // as it's shared within the ActorSystem, no new Counter is instantiated here
  println(counter1.equals(counter2)) // true, i.e. the same instance
  println(counter2.equals(counter3)) // true, i.e. the same instance
  println(Counter(system).increment) // 1
  // note you don't need to call the above get() to instantiate the Extension, you can simply use Counter(system) that is tied to a single ActorSystem, shared within that ActorSystem
  println(Counter(system).increment) // 2
  system.terminate()

  // 2) loaded at ActorSystem creation time through the Akka configuration
  //    ex. akka.extensions = ["akka_extensions.Counter"]
  ...
  // ex. akka.library-extensions += "akka_extensions.Counter"
